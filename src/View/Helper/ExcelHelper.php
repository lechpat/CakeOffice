<?php
namespace CakeOffice\View\Helper;
...
class ExcelHelper extends Helper
{
    ...
    {
        $index = $this->PhpExcel->getSheetCount();
    ...
    {
        $this->_maxRow = $this->PhpExcel->setActiveSheetIndex($sheet)->getHighestRow();
        $this->_row = 1;
    ...
    {
        return call_user_func_array([$this->PhpExcel, $name], $arguments);
    }
    ...
    {
        $this->PhpExcel->getDefaultStyle()->getFont()->setName($name);
    ...
    public function addTableHeader($data, $params = [])
    {
        ...
        if (isset($params['font']))
            $this->PhpExcel->getActiveSheet()->getStyle($this->_row)->getFont()->setName($params['font']);

        // font size
        if (isset($params['size']))
            $this->PhpExcel->getActiveSheet()->getStyle($this->_row)->getFont()->setSize($params['size']);

        // bold
        if (isset($params['bold']))
            $this->PhpExcel->getActiveSheet()->getStyle($this->_row)->getFont()->setBold($params['bold']);

        // italic
        if (isset($params['italic']))
            $this->PhpExcel->getActiveSheet()->getStyle($this->_row)->getFont()->setItalic($params['italic']);
        ...
        foreach ($data as $d) {
            // set label
            $this->PhpExcel->getActiveSheet()->setCellValueByColumnAndRow($offset, $this->_row, $d['label']);

            // set width
            if (isset($d['width']) && is_numeric($d['width']))
                $this->PhpExcel->getActiveSheet()->getColumnDimensionByColumn($offset)->setWidth((float)$d['width']);
            else
    ...
    public function addTableRow($data)
    {
        ...
        foreach ($data as $d)
            $this->PhpExcel->getActiveSheet()->setCellValueByColumnAndRow($offset++, $this->_row, $d);
    ...
    public function addTableFooter()
    {
        ...
        if (count($this->_tableParams['filter']))
            $this->PhpExcel->getActiveSheet()->setAutoFilter(
                \PHPExcel_Cell::stringFromColumnIndex($this->_tableParams['filter'][0]) . ($this->_tableParams['header_row']) . ':' . 
                \PHPExcel_Cell::stringFromColumnIndex($this->_tableParams['filter'][count($this->_tableParams['filter']) - 1]) . ($this->_tableParams['header_row'] + $this->_tableParams['row_count']));

        // wrap
        foreach ($this->_tableParams['wrap'] as $col)
            $this->PhpExcel->getActiveSheet()->getStyle(
                \PHPExcel_Cell::stringFromColumnIndex($col) . ($this->_tableParams['header_row'] + 1) . ':' . 
                \PHPExcel_Cell::stringFromColumnIndex($col) . ($this->_tableParams['header_row'] + $this->_tableParams['row_count']))->getAlignment()->setWrapText(true);
    ...
    public function getTableData($max = 100)
    {
        ...
        for ($col = 0; $col < $max; $col++)
            $data[] = $this->PhpExcel->getActiveSheet()->getCellByColumnAndRow($col, $this->_row)->getValue();
    ...
    {
        return \PHPExcel_IOFactory::createWriter($this->PhpExcel, $writer);
    }
